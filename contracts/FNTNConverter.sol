...
/*
 * The subset of the interface to the FNTN contract that's relevant
 * to our interests. PRs welcome 
 */
...
/*
 * Utility contract to:
 * - Encapsulate common interface functions for convenience
 * - Handle conversion between the token IDs in the original
 *   shared contract that FNTN was in, and the numbers in the
 *   title of FNTN shards. There are functions for both directions.
 */
contract FNTNConverter {
  /*
   * FNTN Contract
   * Call it with code like require(fntnContract.ownerOf(tokenId) == msg.sender)
   */ 
   ...
   * Given the id of a shard (the number in the title, eg "FNTN // 62")
  ...
  function shardIdToTokenId(uint shardId) public pure returns (uint) {
    ...
    require(shardId > 0 && shardId < 176, "Enter a shardId from 1 to 175");
  ...
  /*
   * The tokenIds are not contiguous in the original contract.
   * Given the tokenId in the original shared contract, this outputs the
   * shard id (the number in the title, eg "FNTN // 62").
   */
  function tokenIdToShardId(uint tokenId) public pure returns (uint) {
    // Check up front for a valid id. Saves gas on failure, but also on valid
    // shardIds we can save some gas by not needing SafeMath function calls
    require(tokenId > 1228 && tokenId < 1421, "Enter a tokenId from 1229 to 1420");
